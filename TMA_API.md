...
пользователю, поэтому одну кнопку могут открыть разные пользователи. Такая
ссылка является временным ключом доступа к контексту чата, и её нельзя
пересылать за пределы этого чата.
...
- `503` — обязательный bot token не настроен на сервере.
