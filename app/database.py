...
from app.database_schema import initialize_database_schema

UTC = timezone.utc
WEATHER_LOCATION_CACHE_RETENTION = timedelta(days=30)
_REQUIRED_DATABASE_HEALTH_QUERIES = (
    "SELECT 1 FROM reminders LIMIT 1",
    "SELECT 1 FROM chat_settings LIMIT 1",
    "SELECT 1 FROM reminder_completion_occurrences LIMIT 1",
    "SELECT 1 FROM reminder_delivery_occurrences LIMIT 1",
)
...
def check_required_database_schema() -> None:
    with get_connection() as connection:
        for query in _REQUIRED_DATABASE_HEALTH_QUERIES:
...
def get_all_active_weather_reminders() -> list[sqlite3.Row]:
    return fetch_active_reminders(
        "AND reminder_kind = ?",
        (REMINDER_KIND_WEATHER,),
    )
