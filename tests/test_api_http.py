...
from app.api_auth import TMA_INIT_DATA_HEADER
...
def test_update_tma_reminder_rejects_unknown_reminder(
    authenticated_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    app.state.bot = object()
...
@pytest.mark.parametrize(
    "path",
    [
        "/api/tma/reminders/0?expected_revision=1",
        (f"/api/tma/reminders/{api_module.SQLITE_INT64_MAX + 1}?expected_revision=1"),
        "/api/tma/reminders/42?expected_revision=0",
        (f"/api/tma/reminders/42?expected_revision={api_module.SQLITE_INT64_MAX + 1}"),
    ],
)
def test_delete_tma_reminder_rejects_values_outside_sqlite_int64_range(
    authenticated_client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
    path: str,
) -> None:
